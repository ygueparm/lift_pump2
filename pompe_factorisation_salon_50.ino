...
void setup() {
  ...
  // Charger le dernier démarrage sauvegardé (si disponible)
  dernierDemarrageRecupere = prefsPompe.getULong("dernierDemarrage", 0);  // Récupère la valeur
  ...
  //Initialiser le serveur web en fonction de l'état
    if (relaisDeclenche = true) {
       initWebAlarm(server);  // Initialiser la page d'alerte
   } else {
  ...
  if (peutDemarrerPompe(capteurs) && !moteurEnMarche) {
    ...
    if (moteurEnMarche) {
      ...
      //arret automatique après un certain temps
        ...
        prefsPompe.putULong("dernierDemarrage", dernierDemarrage);  // Sauvegarde le dernier démarrage
        sauvegarderTableau();
      }
    }
